...
graph_router = Agent(
    ...
    system_prompt="You are the entry coordinator. Read the user's market request and pass the target company ticker downstream."
)

initial_synthesis_agent = Agent(
    name="initial_synthesizer",
    model=BedrockModel(model_id=AGENT_MODEL_ID, region_name=REGION),
    system_prompt="Compile the raw data feeds from SEC, Stock, and News into a foundational market brief."
)

revision_agent = Agent(
    name="report_revision_loop",
    model=BedrockModel(model_id=AGENT_MODEL_ID, region_name=REGION),
    system_prompt="""You are a Report Editor. 
Your job is to read the market brief alongside feedback from the Risk Auditor. 
Revise and update the document layout to address the compliance critiques completely."""
)
...
def build_intelligence_graph():
    ...
    builder.add_node(initial_synthesis_agent, "initial_synthesizer")
    builder.add_node(revision_agent, "report_revision_loop")
    ...
    builder.add_edge("graph_router", "stock_fetcher")
    builder.add_edge("graph_router", "news_summarizer")

    # 3. Parallel Join Barrier (Merges into Initial Synthesis)
    builder.add_edge("sec_extractor", "initial_synthesizer")
    builder.add_edge("stock_fetcher", "initial_synthesizer")
    builder.add_edge("news_summarizer", "initial_synthesizer")

    # 4. Progress out of the barrier into the sequential revision environment
    builder.add_edge("initial_synthesizer", "report_revision_loop")
    builder.add_edge("report_revision_loop", "risk_auditor")

    # 5. Safe, Linear Cyclic Feedback Loop
    # Loops back safely below the parallel merge step, preventing execution deadlocks!
    builder.add_edge(
        "risk_auditor", 
