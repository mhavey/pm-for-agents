...
table = dynamodb.Table("trusteeship_process_state")
...
def log_trace_event(
    ...
    try:
        ...
        print(f"Successfully updated/created session {session_id}")
        return response
...
class TraceableNodeAgent(Agent):
    def __init__(self, node_name, session_id, user_id, **kwargs):
        print("MYTRACER_AgentClassinit1")
        kwargs["name"] = node_name
        super().__init__(**kwargs)
        self.node_name = node_name
        self.session_id = session_id
        self.user_id = user_id
        print("MYTRACER_AgentClassinit2")

    def __call__(self, *args, **kwargs):
        """
        Intercepts graph node invocation boundaries dynamically.
        Bridges the contextvars vacuum and logs the execution tracking data to DynamoDB.
        """
        print(f"MYTRACER_AgentClass_runlogic {self.node_name} ___")
        start_ts = datetime.datetime.now()
        
        try:
            # Safely invoke the true framework executor base class loop
            result = super().__call__(*args, **kwargs)
            return result
        finally:
            end_ts = datetime.datetime.now()
            # Calculate duration in seconds (or milliseconds depending on your chart setup)
            duration_seconds = (end_ts - start_ts).total_seconds()
            print(f"___ END NODE: {self.node_name} ({duration_seconds}s) ___")
            
            # Fire the process mining trace log synchronously to your DynamoDB table
            try:
                log_trace_event(
                ...
                log.error(f"Failed to push process mining event to DynamoDB: {ddb_err}")
