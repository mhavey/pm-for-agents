...
class TraceableNodeAgent(Agent):
    ...
    def run(self, state):
        ...
        print(f"AGENT RUN {self.node_name} {self.session_id} {self.user_id}")
        tracer = trace.get_tracer(__name__)
        # Grab the global context to bridge the graph's internal thread gaps
        ...
        with tracer.start_as_current_span(
            ...
            if span.is_recording():
                ...
                # This prefix is CRITICAL for X-Ray searchability
                span.set_attribute(f"aws.xray.annotations.node_name", self.node_name)
                
                # RESTORE PAYLOADS: Manually attach input to this span 
                # so process mining can see WHAT the node received.
                span.set_attribute("input.value", str(state))

            # Run the actual agent logic
            result = super().run(state)
            
            # RESTORE PAYLOADS: Attach output
            if span.is_recording():
...
def _make_node_agent(
    ...
    session_id: str,
    user_id: str,
    history_context: str = "" # Add this parameter
) -> Agent:
