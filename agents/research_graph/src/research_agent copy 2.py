...
class TraceableNodeAgent(Agent):
    def __init__(self, node_name, session_id, user_id, **kwargs):
        ...
        # CRITICAL: Capture the active trace context from the main thread during instantiation
        self.captured_context = context.get_current()

    def _execute_with_trace(self, func, *args, **kwargs):
        tracer = trace.get_tracer(__name__)
        
        # Explicitly pass the captured context to link this span to the root trace
        with tracer.start_as_current_span(
            ...
            if span.is_recording():
                ...
                # Attach payload tracking data
                if args:
                    span.set_attribute("input.value", str(args))
                elif 'state' in kwargs:
                    span.set_attribute("input.value", str(kwargs['state']))

            # Re-inject the context into the active thread scope so that auto-instrumented 
            # downstream spans (BedrockRuntime, tools) attach as children of this node span.
            token = context.attach(context.set_value(trace._SPAN_KEY, span, self.captured_context))
            try:
    ...
    def run(self, state):
        return self._execute_with_trace(super().run, state)

    def __call__(self, *args, **kwargs):
        return self._execute_with_trace(super().__call__, *args, **kwargs)
...
def instrument_graph_runtime(graph_instance):
    """
    Production telemetry patch. Safely wraps graph node invocation boundaries
    without contaminating the core Agent or Routing Predicate business logic.
    """
    # Grab all active nodes managed by the Strands Graph instance
    # Depending on your Strands version, this could be graph_instance.nodes or graph_instance._nodes
    nodes = getattr(graph_instance, "nodes", getattr(graph_instance, "_nodes", {}))

    for node_name, node_agent in nodes.items():
        # We only need to instrument the nodes, tools are handled by the @tool decorator
        if node_name not in {"classify", "search", "browse", "web", "deep", "synth"}:
            continue
            
        # Capture the original execution function pointer
        original_callable = node_agent.__call__

        def make_traced_closure(name, target_callable):
            def traced_call(*args, **kwargs):
                tracer = trace.get_tracer(__name__)
                # Fetch the true thread-local context variables safely
                ...
                with tracer.start_as_current_span(
                    ...
                    if span.is_recording():
                        ...
                        span.set_attribute("aws.xray.annotations.node_name", name)
                        if args:
                            span.set_attribute("input.value", str(args))

                    # Seamless context injection across the async/thread boundary
        ...
        # Apply the wrapper dynamically at runtime
        node_agent.__call__ = make_traced_closure(node_name, original_callable)
...
def invoke(payload: dict, context: Any = None) -> dict:
    ...
    # Clean Telemetry Bootstrapping: Patch the graph boundaries right before execution
    instrument_graph_runtime(graph)
